...
```typescript
interface Room {
  id: string;          // 6位數字的唯一識別碼
  name: string;        // 房間名稱
  maxPlayers: number;  // 最大玩家數
  createdAt: string;   // 建立時間
  lastActivity: string; // 最後活動時間
}
```
...
1. **自動清理機制**
   - 系統每分鐘執行一次清理檢查
   - 房間移除條件：
     - 房間內沒有玩家，且最後活動時間超過 5 分鐘
     - 房間內沒有玩家，且創建時間超過 10 分鐘

2. **手動刪除**
   - 通過 API 端點 `/api/rooms` 的 DELETE 方法
   - 需要提供 `roomId` 來指定要刪除的房間

3. **玩家離開處理**
   - 當玩家斷開連線時，自動從房間中移除
   - 如果房間變成空房間，且符合自動清理條件，就會被移除

4. **活動時間更新**
   - 每次房間有活動時（玩家加入、離開、發送訊息等）更新 `lastActivity`
   - 長時間無活動的房間會被自動清理機制移除
